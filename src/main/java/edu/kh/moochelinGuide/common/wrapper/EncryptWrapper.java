package edu.kh.moochelinGuide.common.wrapper;
...
public class EncryptWrapper extends HttpServletRequestWrapper {
	...
	public String getParameter(String name) {
		...
		// super.getParameter(name) : 부모의 겟파라미터 == 기존 겟파라미터 메서드
	...
	// 해시함수 : 어떤 문자열이든 일정한 길이의 무작위 문자열로 변환하는 함수(중복X)
	private String getSha512(String pw) {
		// 매개변수 pw : 암호회 되기 전 비밀번호
		
		String encryptPw = null; // 암호화된 비밀번호 저장 변수
		
		// 1. 해시 함수를 수행 할 객체를 참조할 변수 선언
		...
		try {
			...
			// 3. md를 이용해 암호화를 진행 할 수 있도록 pw를 byte[] 형태로 변환
