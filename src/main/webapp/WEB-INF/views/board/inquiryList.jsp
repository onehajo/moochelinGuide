...
<link rel="stylesheet"
	href="${contextPath}/resources/css/inquiryList.css">
			...
			<table class="inquiry-list">
				...
				<thead>
					<tr>
						<td>제목</td>
						<td>만든시간</td>
						<td><a href=""><span class="activity"><button
										id="activity">마지막 활동</button></span><label id="triangle-down"
								for="activity">&#x25BC;</label></a></td>
						<td>상태</td>
					</tr>
				</thead>
				<tbody>
					<c:if test="${!empty boardList}">
						<c:forEach var="board" items="${boardList}">
							<tr>
								<td><form action="inquiryContent.html">
										<button class="list-name">${board.boardTit}</button>
									</form></td>
								<td>${board.msg}</td>
								<td>${board.msg2}</td>
