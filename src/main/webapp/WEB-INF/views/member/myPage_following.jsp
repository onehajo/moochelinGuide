...
<link rel="stylesheet"
	href="${contextPath}/resources/css/myPageFollow-style.css">
<!-- 헤더푸터 CSS 연결-->
<link rel="stylesheet"
				...
				<div class="follower">
					<ul class="follower-row">
							...
							<c:forEach var="f" items="${fList}">
								<li>
									<div class="follower-img">
										<a href="#"> <!-- 프로필 경로 없는 경우--> <c:if
												test="${ empty f.profileImage}">
												<img src="${contextPath}/resources/images/user.png"
													class="profile-img">
											</c:if> <!-- 변경 이미지 --> <c:if test="${ !empty f.profileImage}">
												<img src="${contextPath}${f.profileImage}"
													class="profile-image">
											</c:if>
										</a>
									</div>
									<div class="follower-info">
										<span>${f.memberName}</span> <span>평가한 영화 수 :
											${f.evaluationCount}</span>
									</div>
						...
						<c:if test="${empty fList}">
							<div id="searchFail">
								<p>내가 팔로우하는 사람</p>
								<p>회원님이 팔로우하는 모든 사람들이 여기에 표시됩니다.</p>
							</div>
						</c:if>
