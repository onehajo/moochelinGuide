	...
	fill: #392eff;
						...
						<c:if test="${empty query}">
							...
							<form action="${contextPath}/search/title" name="search-form">
								<fieldset class="hd-fieldset">
									<i class="fa-solid fa-magnifying-glass" id="search-btn"></i> <input
										type="search" id="query" name="query" autocomplete="off"
										placeholder="제목, 인물, 유저를 검색해보세요">
								</fieldset>
							</form>
						</c:if>
				...
				<c:choose>
					...
					<c:otherwise>
						...
						<li class="profile"><a
							href="${contextPath}/member/profile/my"><div
									id="profileImage"></div></a></li>

						<c:if test="${empty loginMember.profileImage}">
							<li class="profile"><a
								href="${contextPath}/member/myPage/mod"><div
										id="profileImage"></div></a></li>
						</c:if>
						...
						<c:if test="${!empty loginMember.profileImage}">
							<li class="profile"><a
								href="${contextPath}/member/myPage/mod"><div
										id="profileImage"
										style=" background-size: 40px; background-image : url(${contextPath}${loginMember.profileImage})"></div></a></li>
						</c:if>
...
<div id="login-box" class="popup">
	...
	<form action="login" method="post" id="login-form" class="form-css"
	...
	<div class="login-other">
		<div>
			<a onclick="openPw()">비밀번호를 잊어버리셨나요?</a>
		</div>
		<div>
			계정이 없으신가요?<a onclick="openPopSignUp()">회원가입</a>
		</div>
	</div>
	<hr class="line"></hr>
...
<form action="signUp" method="post" id="signup-form"
	...
	<div id="signup-box" class="popup">
		...
		<div class="login-other">
			<div>
				이미 가입하셨나요?<a onclick="openPopLogin()"> 로그인</a>
			</div>
		</div>
		<hr class="line"></hr>
		<!-- <div class="sns" id="GgCustomLogin">
	...
	<section id="agree" class="popup" style="padding: 0;">
		<div class="agree-title">
			약관에 동의하시면<br>가입이 완료됩니다.
		</div>
		<div class="agree-all">
			<input type="checkbox" id="agreeAll" name="agreeCheck"
				onclick="selectAll(this)">전체 약관 동의
		</div>
		<ul>
			<li>
				<div>
					<input type="checkbox" id="agree1" name="agreeCheck">서비스
					이용약관
				</div>
				<button>보기</button>
			</li>
			<li>
				<div>
					<input type="checkbox" id="agree2" name="agreeCheck">개인정보
					처리방침
				</div>
				<button>보기</button>
			</li>
			<li>
				<div>
					<input type="checkbox" id="agree3" name="agreeCheck">신작 알림,
					이벤트 정보 수신(선택)
				</div>
				<button>보기</button>
			</li>
...
<div id="pw-box" class="popup">
	...
	<form action="pwfind" method="post" class="form-css"
		onsubmit="return pwfindValidate()">
		<input type="text" placeholder="이메일" id="pwFindEmail"
			name="memberEmail">
